...
:root {
	--app-mdmanager-primary: #2ea256;
	--app-mdmanager-danger: #ff4e54;
	--app-mdmanager-warning: #ff9800;
...
:root[data-darkmode="true"] {
	--app-mdmanager-primary: #2ea256;
	--app-mdmanager-danger: #a93734;
	--app-mdmanager-warning: #f57c00;
...
return view.extend({
    ...
    callRpcd: function(method, params) {
        return new Promise((resolve, reject) => {
            L.resolveDefault(fs.exec('/bin/ubus', ['call', 'diskmanager', method, JSON.stringify(params)]))
                .then(res => {
                    if (res && res.code === 0) {
                        try {
            ...
            .then(res => {
                if (res && res.code === 0) {
                    if (res.stdout.match(/PASSED/i)) {
                        return { status: _('PASSED'), color: 'var(--app-mdmanager-primary)' };
                    } else if (res.stdout.match(/FAILED/i)) {
                        return { status: _('FAILED'), color: 'var(--app-mdmanager-danger)' };
                    }
                }
    ...
    parsePartedOutput: function(output) {
        ...
        for (let line of lines) {
            if (line.includes('Disk /dev/')) {
                let match = line.match(/:\s*([\d]+)B/);
                if (match) diskSize = parseInt(match[1], 10);
            }
            ...
            if (inPartitionTable && line.trim()) {
                ...
                if (parts.length >= 4) {
                    ...
                    let partition = {
                        number: number,
                        start: parseInt(parts[1].replace('B', ''), 10),
                        end: parseInt(parts[2].replace('B', ''), 10),
                        size: parseInt(parts[3].replace('B', ''), 10),
                        type: 'primary',
                        filesystem: '',
                        flags: ''
                    };
    ...
    renderPartitionBar: function(diskInfo) {
        ...
        for (let i = 0; i < partitions.length; i++) {
            ...
            if (isUnallocated && part.size < 1048576) {
                continue;
            }
    ...
    renderPartitionTable: function(diskInfo) {
        ...
        if (!diskInfo.partitions || diskInfo.partitions.length === 0) {
            let rowsEmpty = [[E('em', {}, _('No partitions found')), '', '', '', '', '', '', '']];
            if (typeof cbi_update_table === 'function') {
                cbi_update_table(table, rowsEmpty);
            } else {
                let tbody = E('tbody', {});
                let tr = E('tr', {'class': 'placeholder'}, E('td', {'colspan': partitionTableTitles.length}, E('em', {}, _('No partitions found'))));
                ...
                if (isExtended && extendedPartitions[partNum]) {
                    ...
                    extendedPartitions[partNum].forEach(function(logPart) {
                        ...
                        let logTypeCell = '<span class="partition-color-indicator ' + logIndicatorClass + '"></span> ' + 
                                         (logFsType || shortTypeLabels['logical']);
    ...
    updateActionButtons: function() {
        if (!this.selectedDisk) {
            ['btn-mount', 'btn-unmount', 'btn-create', 'btn-delete', 'btn-format'].forEach(id => {
    ...
    showCreatePartitionDialog: function() {
        ...
        this.getDiskInfo(this.selectedDisk).then(async (diskInfo) => {
            ...
            if (totalUnallocated === 0 && diskInfo.hasPartitionTable) {
            ...
            let refreshPartitionData = function() {
                ...
                this.getDiskInfo(this.selectedDisk).then(freshDiskInfo => {
                    ...
                    let newFreeSpaceMB = Math.floor(newTotalUnallocated / (1024 * 1024));
            ...
            setTimeout(() => {
                ...
                if (supported && supported.length > 0) {
                    ...
                    ordered.forEach((fsName, idx) => {
                        let label = fsLabelMap[fsName] || fsName;
    ...
    showFormatDialog: function() {
        ...
        this.detectSupportedFilesystems().then((supported) => {
            ...
            if (supported && supported.length > 0) {
                ...
                const ordered = preferred.filter(x => supported.indexOf(x) !== -1).concat(supported.filter(x => preferred.indexOf(x) === -1));
                ordered.forEach((fsName, idx) => {
                    fsOptions.push(E('option', {'value': fsName, ...(idx===0?{'selected':'selected'}:{})}, fsLabelMap[fsName] || fsName));
                });
            } else {
                ...
                E('table', {'class': 'table'}, [
                    ...
                    E('tr', {'class': 'tr'}, [
                        ...
                        E('td', {'class': 'td left'}, 
                            ...
                            E('span', {'style': 'color: var(--app-mdmanager-primary)'}, _('Mounted')) : 
